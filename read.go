...
func (app *App) FetchPublicPosts() (interface{}, error) {
	...
	rows, err := app.db.Query(`SELECT p.id, c.id, alias, c.title, p.slug, p.title, p.content_trunc, p.text_appearance, p.language, p.rtl, p.created, p.updated, p.reddit_url
	FROM collections c
	...
	for rows.Next() {
		...
		err = rows.Scan(&p.ID, &c.ID, &alias, &title, &p.Slug, &p.Title, &p.Content, &p.Font, &p.Language, &p.RTL, &p.Created, &p.Updated, &p.RedditUrl)
		if err != nil {
